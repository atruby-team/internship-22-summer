...
class BbcNews 
  ...
  def self.add_bbc_news(bbc_new)
    @@bbc_news.push(bbc_new)
  end
  ...
  def self.update_bbc_news(bbc_new, title, content)
    return unless @@bbc_news.include?(bbc_new)

    index = @@bbc_news.index(bbc_new)
    @@bbc_news[index].title = title
    @@bbc_news[index].content = content
  end

  def self.delete_bbc_news(bbc_new)
    return unless @@bbc_news.include?(bbc_new)
    @@bbc_news.delete(bbc_new)
  end
end
...
BbcNews.add_bbc_news(bbc_news_01)
BbcNews.add_bbc_news(bbc_news_02)
BbcNews.add_bbc_news(bbc_news_03)
# p BbcNews.get_bbc_news
BbcNews.update_bbc_news(bbc_news_02, "BBC second news!", "Hello Ruby from Asiantech")
# p BbcNews.get_bbc_news
BbcNews.delete_bbc_news(bbc_news_01)
# p BbcNews.get_bbc_news
...
class CnnNews 
  ...
  def self.add_cnn_news(cnn_new)
    @@cnn_news.push(cnn_new)
  end
  ...
  def self.update_cnn_news(cnn_new, title, content)
    return unless @@cnn_news.include?(cnn_new)

    index = @@cnn_news.index(cnn_new)
    @@cnn_news[index].title = title
    @@cnn_news[index].content = content
  end

  def self.delete_cnn_news(cnn_new)
    return unless @@cnn_news.include?(cnn_new)

    @@cnn_news.delete(cnn_new)
  end
end
...
CnnNews.add_cnn_news(cnn_news_01)
CnnNews.add_cnn_news(cnn_news_02)
CnnNews.add_cnn_news(cnn_news_03)
# p CnnNews.get_cnn_news
CnnNews.update_cnn_news(cnn_news_02, "CNN second news!", "Hello Ruby from Asiantech")
# p CnnNews.get_cnn_news
BbcNews.delete_bbc_news(cnn_news_03)
# p CnnNews.get_cnn_news
...
class User 
  ...
  def friends
    array = @friends.map { |x| x = x.name }
    result = @friends.map do |x|
  ...
  def send_message(friend, message)
    ...
    @messages.push({self.name => message})
  ...
  def conversation(friend)
    ...
    index = @friends.index(friend)
    result = @messages.keep_if do |x|
                x.key?(self.name) || x.key?(array[index])
             end
  end
end
